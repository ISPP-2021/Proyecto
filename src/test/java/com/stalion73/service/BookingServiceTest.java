...
public class BookingServiceTest {
	...
	void findAllBookingTest() {
		...
		Assertions.assertThat(bookings.isEmpty()).isFalse();
		Assertions.assertThat(bookings.size()).isEqualTo(count);
	}

	@Test
	void findBookingByIdTest() {
		Optional<Booking> booking = this.bookingService.findById(1);
		Assertions.assertThat(booking.get().getStatus()).isEqualTo(Status.IN_PROGRESS);
	}

	@Test
	void findBookingByStatusTest() {
		Collection<Booking> booking = bookingService.findBookingByStatus(Status.IN_PROGRESS);
		Assertions.assertThat(booking.size()).isEqualTo(3);
	}
	...
	void saveBookingTest() {
		...
		Assertions.assertThat(booking.getBookDate()).isEqualTo(new GregorianCalendar(2021, 07, 07).getTime());
		Assertions.assertThat(booking.getEmisionDate()).isEqualTo(new GregorianCalendar(2021, 06, 06).getTime());
		Assertions.assertThat(booking.getStatus()).isEqualTo(Status.IN_PROGRESS);
	}
	...
	void deleteBookingByIdTest() {
		...
		List<Booking> bookings = (List<Booking>) this.bookingService.findAll();
		Assertions.assertThat(bookings.size()).isEqualTo(count - 1);
	}
